...
title: "GitLab"
icon: GitLab
description: "Connect GitLab.com or your own instance, give each agent its own bot account, and trigger from issues and merge requests."
---
...
There is no separate "install on this project" step. A project is set up the moment you first give it to an agent — as its [workspace](/build-your-team/agents/workspaces-and-repos), or as an authorized additional repository. Setting it up does three things:
...
<Image src={imgGitlabCodeHosts} alt="The GitLab card under Integrations → Code hosts: the administration connection, then one row per agent with the groups its service account belongs to" width={800} />
...
<Image src={imgGitlabAddIntegration} alt="Add integration with GitLab selected: pick a project, choose issues or merge requests, and how eagerly the agent wakes up" width={560} />
...
Choose the project, what to listen for — issues, merge requests, releases, or any mix — and how eagerly the agent wakes up:

- **opened** — when an issue or merge request is opened, plus later explicit mentions in that family.
- **any update** — openings plus new revisions, labels and replies.
- **@-mention** — when the agent is @-mentioned.

A release offers only **published**.

<Image src={imgGitlabAgentCard} alt="An agent's GitLab card with three watched projects, each showing the merge-request family and its trigger" width={640} />
...
Each qualifying event starts a session on the agent's daemon with the event as context, and the sessions appear in [Sessions](/operate-govern/sessions) linked back to GitLab.
