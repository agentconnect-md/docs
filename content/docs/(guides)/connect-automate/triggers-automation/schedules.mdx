...
title: "Schedules"
icon: CalendarClock
description: "Run an agent on a timer — daily reports, nightly audits, periodic sweeps — with output posted to a channel or kept headless."
---

import imgSchedulesList from '@/public/images/schedules-list.png'

**Schedules** run an agent on a cron cadence: a daily deploy report, a nightly dependency audit, a Monday-morning triage sweep. Each firing starts a fresh session for the agent, so every run is fully replayable in [Sessions](/operate-govern/sessions).

![Create a schedule](/images/new-schedule.png)
...
- **Visibility** — who in the org sees the schedule ([details](/operate-govern/permissions-overview/team-visibility)).
...
<Image src={imgSchedulesList} alt="Scheduled jobs with their cron, last run, next run, and on/off toggle" width={900} />
...
- Because each run is a fresh session, the prompt must be self-contained — use the agent's [memory](/build-your-team/agents/configure-an-agent) for standing context.
