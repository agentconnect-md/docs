...
title: "Webhooks"
icon: Webhook
description: "An HTTPS endpoint that turns any POST into an agent session — with an HMAC signature to verify it's really you."
---
...
<Image src={imgWebhookIntegration} alt="Create a webhook integration" width={640} />
...
Each delivery starts (or continues) a session on the agent's daemon — you'll find it in [Sessions](/operate-govern/sessions) with the webhook as its trigger, full transcript included. The agent's page shows the webhook's recent deliveries and when it last fired.
...
- Pair webhooks with a [GitHub workspace](/build-your-team/agents/workspaces-and-repos) so the agent can actually inspect the code it's being told about.
