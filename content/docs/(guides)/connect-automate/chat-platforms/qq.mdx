...
title: "QQ"
icon: QQ
description: "Connect an official QQ bot for private chats and group @mentions on a self-hosted deployment."
---

QQ is available on **self-hosted deployments only**. AgentConnect Cloud does not offer it. The Helm chart turns it on by default (`features.qq`); on Docker Compose, add `qq` to `FEATURE_FLAGS` in [`compose.env`](/self-hosting/docker-compose/configuration#compose-environment) and recreate `web`. Until the flag is on, QQ does not appear in the integration picker.
...
See [In-conversation commands](/connect-automate/integrations-overview#in-conversation-commands) for stopping, cancelling, queuing, and changing supported session settings.
