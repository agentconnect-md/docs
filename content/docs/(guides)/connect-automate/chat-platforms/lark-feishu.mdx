...
title: "Lark / Feishu"
icon: Lark
description: "Connect a Lark or Feishu bot with one-click or manual setup."
---
...
<Image src={imgLarkFeishuIntegration} alt="Create a Feishu or Lark bot with the one-click setup" width={640} />
...
Each agent bot may use its own platform App ID. The deployment keeps one regional Login App as a tenant anchor, so it can admit multiple bot Apps from the same trusted workspace without requiring their App IDs to match. When **Follow Feishu / Lark access** is enabled, AgentConnect checks a session's current chat membership through that session's installed bot App. See [Lark and Feishu identities](/self-hosting/sign-in#lark-and-feishu-identities) for the self-hosted identity setup.
...
- Use the [in-conversation commands](/connect-automate/integrations-overview#in-conversation-commands) to stop, cancel, queue, or inspect a session.
