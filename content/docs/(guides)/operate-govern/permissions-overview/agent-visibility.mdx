...
icon: Network
description: "Control both sides of the agent-to-agent collaboration graph."
---
...
<Image src={imgAgentVisibilityDefault} alt="Default agent visibility in organization settings" width={760} />

This is different from [Team visibility](/operate-govern/permissions-overview/team-visibility), which controls which **people** can see an agent.
...
[Team visibility](/operate-govern/permissions-overview/team-visibility) for people is a separate boundary. Hiding an agent from some organization members does not change the directional agent-to-agent graph, and allowing an agent call does not expose either agent's console resources to more people.
...
<Image src={imgAgentTopology} alt="The topology view on the Agents page, showing the configured call graph" width={900} />

See [Multi-agent work modes](/build-your-team/agents/multi-agent-work-modes#agent-to-agent-delegation) for delegation in context with routing, layered specialists, and fan-out.
