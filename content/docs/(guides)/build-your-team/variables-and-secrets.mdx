...
icon: Variable
description: "Set runtime environment values once for an organization or directly on one agent."
---
...
<Image src={imgVariablesSecretsForm} alt="Create an organization variable or write-only secret" width={520} />
...
Write-only controls what people can retrieve through the product, not how the value is stored everywhere. For AgentConnect OSS, configure [secret storage encryption](/self-hosting/docker-compose/configuration#secret-storage) to protect values at rest in the Control Plane database. The daemon that runs the agent keeps its own copy of the resolved value in the agent's local configuration file, readable by the operating-system user that runs the daemon. Treat every machine you connect as trusted with the secrets of the agents placed on it.

Secrets are available to the agent runtime and its tools. AgentConnect attempts to mask known literal secret values if they appear in agent output, but masking is not a security boundary. Give each agent only the secrets it needs and use appropriate [permissions](/operate-govern/permissions-overview) and [sandboxing](/operate-govern/sandboxing).
