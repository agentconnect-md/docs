...
[Dreaming](/build-your-team/agents/configure-an-agent#dreaming) can turn recurring patterns from an agent's recent work into a proposed Knowledge entry or managed skill. A proposal never publishes itself:
...
An accepted Knowledge suggestion appears in the Knowledge library. An accepted managed skill appears in **Tools & Skills → Skills library** and still must be [enabled explicitly for each agent](/build-your-team/tools-and-skills#enable-tools-and-skills-for-an-agent).
...
Follow [the guide to using Mem0 OSS as external memory](/how-to-guides/external-memory) for a complete deployment, connection, binding, and recall test.
