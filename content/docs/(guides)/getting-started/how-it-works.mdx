...
title: "How it works"
icon: Workflow
description: "Agent execution stays on the daemons you operate while the Control Plane coordinates the fleet."
---
...
<Image src={imgHowAgentconnectWorks} alt="AgentConnect channels and events reach agents running on a daemon, while the Control Plane coordinates over a separate control path" width={920} />
...
**The Relay** is optional public ingress. It terminates callback-based Slack and Lark / Feishu traffic, GitHub, GitLab and generic webhooks, [Linear](/connect-automate/triggers-automation/linear) agent events, and webchat (the browser transport behind the [Playground](/build-your-team/agents/playground)), then forwards the request to the owning daemon. It does not durably store message content.

**The Control Plane** manages authentication, organizations, permissions, placement, integrations, schedules, and control metadata. It also stores explicitly approved [Knowledge and managed skills](/build-your-team/knowledge) and coordinates the daemon fleet.
