...
title: "Quick start"
icon: Rocket
description: "Create your organization, choose where agents run, and bring your team into the tools where work already happens."
---
...
- Access to [AgentConnect Cloud](https://app.agentconnect.md) or an [AgentConnect OSS](/self-hosting) deployment.
...
<Image src={imgLogin} alt="The AgentConnect sign-in page" width={680} />
...
The foreground command is best for a first run. To keep the daemon running after logout or restart, see [Install the daemon](/build-your-team/daemons/install-the-daemon).
...
<Image src={imgOnboardingChecklist} alt="The completed Getting started checklist with the built-in AgentConnect agent expanded" width={780} />
...
- [Connect platforms](/connect-automate/integrations-overview) — link agents to Slack, Telegram, Discord, Lark / Feishu, GitHub, GitLab, Linear, and other triggers.
- [Create agents](/build-your-team/agents/create-an-agent) — add specialists with different roles, runtimes, models, and workspaces.
- [Permissions](/operate-govern/permissions-overview) — control agent, session, repository, and agent-to-agent access.
- [How it works](/getting-started/how-it-works) — understand the daemon, optional Relay, and Control Plane.
- [AgentConnect OSS](/self-hosting) — run the open-source stack with Docker Compose or Kubernetes.
