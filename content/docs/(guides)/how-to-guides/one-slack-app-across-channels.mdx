...
icon: Route
description: "Use one shared Slack identity while each channel dispatches to an agent with its own model, instructions, workspace, and trigger."
---
...
<Image src={imgSlackIntegration} alt="Create a Slack bot identity that can be shared across agents" width={640} />
...
This is contextual routing from [Multi-agent work modes](/build-your-team/agents/multi-agent-work-modes#shared-bot-with-contextual-routing): one inbound message selects one agent rather than broadcasting to all of them.
...
AgentConnect OSS operators should configure [public Relay URLs](/self-hosting/docker-compose/configuration#network-and-public-urls) before creating the App.
...
If people deliberately bring another shared-bot agent into the same thread, both agents keep their own sessions and configurations. Use [agent visibility](/operate-govern/permissions-overview/agent-visibility) when those agents should also delegate directly to one another.
...
See [Slack](/connect-automate/chat-platforms/slack) for installation details and [Configure an agent](/build-your-team/agents/configure-an-agent) for models, prompts, workspaces, and permissions.
