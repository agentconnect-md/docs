...
icon: Brain
description: "Connect an agent to self-hosted Mem0, choose recall and capture policies, and verify memory across sessions."
---
...
The deployment operator must first complete the [optional Mem0 configuration](/self-hosting/external-memory). You then need:
...
<Image src={imgExternalMemoryConnection} alt="Register the operator-approved Mem0 connection" width={560} />
...
<Image src={imgExternalMemoryPolicy} alt="Choose external-memory recall and capture policy" width={900} />
