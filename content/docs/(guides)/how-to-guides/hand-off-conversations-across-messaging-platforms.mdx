...
icon: ArrowLeftRight
description: "Move work between two messaging workspaces you trust, while keeping each platform's session and audience boundaries explicit."
---
...
<Image src={imgAddIntegration} alt="Connect an agent to approved messaging platforms" width={640} />
...
The same agent owns both sides. When two different agents divide the work, use [agent-to-agent delegation](/build-your-team/agents/multi-agent-work-modes#agent-to-agent-delegation) instead.
...
See [Integrations overview](/connect-automate/integrations-overview) to connect platforms and [Sessions](/operate-govern/sessions) to inspect each side.
