...
AgentConnect Checks block merging only when the repository requires one. [Require a review before merging](/connect-automate/triggers-automation/github#require-a-review-before-merging) explains which reviewers can be required; a **mention only** reviewer such as `deep-review` cannot.
