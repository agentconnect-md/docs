...
icon: GitPullRequest
description: "Review every PR revision with a fast model, then summon a stronger reviewer only when a change needs deeper analysis."
---
...
<Image src={imgGithubIntegration} alt="Configure a pull-request reviewer and its trigger" width={640} />
...
This is the layered-specialist pattern from [Multi-agent work modes](/build-your-team/agents/multi-agent-work-modes#layered-or-on-demand-specialists). Compare it with trigger fan-out and direct delegation before configuring the reviewers.

| Agent | Model profile | Trigger | PR review | Role |
| --- | --- | --- | --- | --- |
| `quick-review` | Fast and efficient | **any update** | **Brief** | Baseline correctness and regressions |
| `deep-review` | Strong reasoning | **@-mention** | **Details** | Architecture, security, migrations, operations |
...
See [GitHub](/connect-automate/triggers-automation/github) for integration behavior and [Workspaces & repositories](/build-your-team/agents/workspaces-and-repos) for repository authorization.
...
3. Set **Trigger when** to **any update**.
...
The **any update** cadence covers opened PRs, new revisions, and supported PR conversation events. Only revision-bearing events open an automatic review generation. If you only want the baseline review when a PR is first opened, choose **opened** instead; later commits will then require an explicit mention.
...
AgentConnect Checks block merging only when the repository requires one. [Require a review before merging](/connect-automate/triggers-automation/github#require-a-review-before-merging) explains which reviewers can be required; an **@-mention** reviewer such as `deep-review` cannot.
...
- A targeted `@deep-review` mention wins over the baseline reviewer's broader **any update** cadence for that delivery.
- Nobody has to remember the exact name. An organization team named `deep-review` makes `@<organization>/deep-review` autocomplete in the comment box and summon the same agent — see [Make the agent name autocomplete](/connect-automate/triggers-automation/github#make-the-agent-name-autocomplete).
...
- PR bodies, diffs, and comments are untrusted input. Use conservative agent permissions, especially on public repositories, and run reviewers of outside contributions in `microsandbox` or Kubernetes ([execution environments](/operate-govern/sandboxing#choose-an-execution-environment)).
...
| Symptom | Check |
| --- | --- |
| `@deep-review` does nothing | Exact agent name; `write`/`admin` author; **@-mention** trigger |
