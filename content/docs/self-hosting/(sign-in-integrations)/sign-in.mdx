...
title: "Sign-in"
icon: LogIn
description: "Add Logto-backed sign-in to AgentConnect OSS with Logto Cloud, external Logto OSS, or the bundled local overlay."
---
...
The Lark and Feishu cards configure regional tenant Apps for trusted-workspace bot admission. They do not automatically add Lark or Feishu as Logto sign-in methods. See [Lark and Feishu tenant Apps](/self-hosting/provider-apps#lark-and-feishu-tenant-apps).
...
Setup does not create the Lark or Feishu Logto connector. If you enable either sign-in method, confirm that its Logto identity record includes `union_id`; an App-scoped `open_id` is not sufficient. See [Lark / Feishu](/connect-automate/chat-platforms/lark-feishu) for the bot permissions and installation flow.
