...
title: "Quick start"
icon: Rocket
description: "Start the AgentConnect OSS stack with Docker Compose and connect a daemon on your own machine."
---

The Docker Compose stack runs AgentConnect on one machine for a local evaluation, with daemons on machines you operate. For a cluster deployment, go to [Deploy with Helm](/self-hosting/kubernetes/deploy).
...
The first run starts PostgreSQL 18, applies the database migrations, initializes the local no-auth organization, and starts the application services. It adds no sample data. The built-in `agentconnect` agent is enabled by default and can be disabled in [Setup](/self-hosting/docker-compose/configuration#setup-server).
...
The daemon connects to the Control Plane and Relay with outbound connections. If it runs on another machine, configure host-reachable URLs first; see [Network and public URLs](/self-hosting/docker-compose/configuration#network-and-public-urls).
...
Continue with [Sign-in](/self-hosting/sign-in) and use Google for the shortest local sign-in path. The local overlay uses a separate Logto database in the same PostgreSQL service and remains bound to loopback.

When you use the overlay, keep both `-f` arguments in later `up`, `pull`, and `down` commands.

For hosted production sign-in, skip the overlay and connect a [Logto Cloud tenant](/self-hosting/sign-in#logto-cloud) instead.
...
- [Sign-in](/self-hosting/sign-in)
- [Install the daemon as a service](/build-your-team/daemons/install-the-daemon)
