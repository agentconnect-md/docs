...
- **By decision**: before each new session starts, a Decision picks which of these repositories the session is about, and only those are checked out; the rest are on demand for that session. It needs a ready Decision provider and a **Repository selector** on the agent (the provider and model that make the pick, set in **Edit workspace**). If the selection cannot run, the session fails to start with an error instead of guessing.
...
An organization owner can also **Authorize an installation** in **Edit workspace**. The agent then gets every repository one GitHub App installation covers, including repositories created later, at one access level. Those repositories are **On demand** by default, so the agent clones what it needs; set the installation to **By decision** to have the Decision pick among them as well. A repository you also authorize on its own keeps its own access and checkout, and informational Checks still need the repository authorized on its own.
