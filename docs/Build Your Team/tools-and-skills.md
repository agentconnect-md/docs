...
If a service you expect is missing from the catalog, it is usually one of two cases. Services AgentConnect integrates directly — GitHub, GitLab, Linear, Slack, Telegram, Discord, and Lark / Feishu — are kept out of the connector catalog by default in favor of their own integrations. Services that authorize solely through OAuth appear once an OAuth client is configured for them in the deployment — on a self-hosted stack, see [Deployment and configuration](/docs/deployment-and-configuration).
