...
Agents on daemons you connect yourself are never billed: they use the provider credentials configured on those machines, and their model traffic flows directly between your machine and your provider.
...
- **Runtime missing from the pickers** — check the daemon service user's host installation and stored login. For microsandbox, also check the selected image: a host installation does not install the guest binary. The Host / Sandbox view separates these states; see [runtime discovery and login status](/docs/sandbox-runtime-support#read-the-runtime-status-in-the-console).
- **Run in sandbox is unavailable** — the selected backend did not pass its live Linux probe. SRT requires `bubblewrap` / `ripgrep` / `socat` and working user namespaces; microsandbox requires usable KVM/vsock access and a compatible image. Check the log and [backend requirements](/docs/sandboxing), then restart after correcting the setup.
