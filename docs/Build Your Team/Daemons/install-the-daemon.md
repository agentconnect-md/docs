...
- **macOS, Linux, or Windows.** Background service mode uses launchd or systemd; on Windows, run the daemon in the foreground. Sandboxing (`srt`, `microsandbox`) is Linux only; elsewhere agents run with `host`.
...
- **Optional Linux sandboxing:** the daemon offers every execution strategy by default and makes each one available only when its requirements are met. SRT requires `bubblewrap`, `ripgrep`, `socat`, and unprivileged user namespaces; on Ubuntu 24.04 and later, [allow user namespaces for bubblewrap](/docs/sandboxing#ubuntu-2310-and-later-allow-user-namespaces-for-bubblewrap) first. microsandbox requires KVM and vsock access. See [Sandboxing](/docs/sandboxing) for setup and [Credential protection](/docs/credential-protection) for login support.
