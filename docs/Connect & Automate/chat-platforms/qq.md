...
QQ is available on **self-hosted deployments only**. AgentConnect Cloud does not offer it. The Helm chart turns it on by default (`features.qq`); on Docker Compose, add `qq` to `FEATURE_FLAGS` in [`compose.env`](/docs/deployment-and-configuration#compose-environment) and recreate `web`. Until the flag is on, QQ does not appear in the integration picker.
