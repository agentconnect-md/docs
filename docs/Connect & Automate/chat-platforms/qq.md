...
QQ is available on **self-hosted deployments only**. AgentConnect Cloud does not offer it. The official Helm chart turns it on by default (`features.qq`). On the Docker Compose stack, add `qq` to `FEATURE_FLAGS` in `compose.env`, then recreate the web service with the same Compose files your deployment already runs — `docker compose restart` keeps the container's current environment and would leave the flag off:

```bash
# base stack
docker compose --env-file compose.env up -d --force-recreate web

# keep every overlay you start the stack with, such as the bundled Logto one
docker compose --env-file compose.env -f compose.yaml -f compose.logto.yaml up -d --force-recreate web
```
