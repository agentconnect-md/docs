...
After a bot discovers a channel, group, or direct message, the conversation appears on the agent's integration. Channels and groups can respond only to mentions, to any message, or not at all. Every observed one-to-one direct message has an independent **On / Off** control for that agent. It starts On for an Everyone agent and Off for a [restricted agent](/docs/visibility-and-sharing#what-a-restricted-agent-changes).
