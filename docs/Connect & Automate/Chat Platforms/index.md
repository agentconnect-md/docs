...
On AgentConnect Cloud, start Slack with the built-in **Add to Slack** flow or connect Lark in one click. Self-hosted deployments can use the same setup for Lark or Feishu. Telegram and Discord begin with a bot token from their own platform.
...
After a bot discovers a channel, group, or gated direct message, the conversation appears on the agent's integration. Channels and groups can respond only to mentions, to any message, or not at all. Direct messages are listed here only for a [restricted agent](/docs/visibility-and-sharing#what-a-restricted-agent-changes), gated with **On** or **Off**.
