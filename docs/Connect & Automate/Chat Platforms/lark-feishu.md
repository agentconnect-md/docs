---
title: 🪽 Lark / Feishu
excerpt: Create a Lark or Feishu bot in one click, with Long Connection by default and HTTP callbacks when needed.
hidden: false
---
...
Lark / Feishu does not offer a bot self-leave action here. A row therefore offers **Remove from this list**, which only hides it in AgentConnect. Remove the bot on the platform to end its membership; if it remains in the chat, a later message can make the row return.
