...
| **A trusted certificate** | There is no skip-verify option at any layer. A private authority is supported by [installing its bundle](#private-certificate-authorities) where every process and sandbox can read it. |
...
**The bot user and its token are not deployment state.** Each organization creates its own bot user, gives it Admin on the repositories its agents will work in, and pastes that user's token in the console under **Integrations → Code hosts → Gitea**; see [Gitea](/docs/gitea). An operator never handles the token.
...
| **A trusted certificate**   | There is no skip-verify option at any layer. A private authority is supported by [installing its bundle](#private-certificate-authorities).                                                                            |
