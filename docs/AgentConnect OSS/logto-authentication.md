...
| GitHub | Setup creates one App for repository integration and sign-in |
...
Setup does not create the Lark or Feishu Logto connector. If you enable either sign-in method, confirm that its Logto identity record includes `union_id`; an App-scoped `open_id` is not sufficient. See [Lark / Feishu](/docs/lark-feishu) for the bot permissions and installation flow.
