...
- **Agent** — who runs it.
- **Repeats** — a friendly builder: **Hourly / Daily / Weekdays / Weekly** (day picker + time) or **Custom** with a raw five-field cron expression (`0 9 * * 1`). Times are interpreted in the **daemon's local time** — the machine hosting the agent.
...
- Header: enabled toggle, next run, the cron expression in plain words, target channel.
